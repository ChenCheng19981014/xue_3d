<template>
  <div class="vue-list-marquee-list">
    ...
    <div class="list-items">
      <template v-if="option.needHover">
        <div
          ...
          :key="index"
          @mouseenter="switchLoop('stop')"
          @mouseleave="switchLoop('start')"
        >
    ...
    <!-- 列表副本dom，用于首尾相连 -->
    <div v-show="listCopyExistFlag" class="list-items-copy">
      <template v-if="option.needHover">
        <div
          v-for="(item, index) in listData"
          :key="index"
...
export default {
  ...
  watch: {
    ...
    listData(newVal, oldVal) {
      this.$nextTick(() => {
        ...
        const listDom = this.$el.getElementsByClassName('list-items')[0]
        const listCopyDom = this.$el.getElementsByClassName('list-items-copy')[0]
  ...
  methods: {
    ...
    hasRestMode(indi) {
      ...
      const listDom = this.$el.getElementsByClassName('list-items')[0]
      const listCopyDom = this.$el.getElementsByClassName('list-items-copy')[0]
      const listBoxDom = this.$el
      const boxHeight = listBoxDom.offsetHeight // list容器的高度；
      const offsetHeightTotal = listDom.offsetHeight // 获取所有条目的总高度；
      if (offsetHeightTotal >= boxHeight) {
        // 所有条目的总高度 大于或等于 list容器的高度，才轮播；
        this.listCopyExistFlag = true
        listDom.style.transition = `transform ${
          this.innerOption.moveTime
        }ms linear`
        listCopyDom.style.transition = `transform ${
          this.innerOption.moveTime
        }ms linear` // TODO：运动函数做出可配置
        if (indi === 'start') {
          clearInterval(this.loopTimer)
          this.loopTimer = setInterval(async() => {
            this.currentItemIndex =
              (this.currentItemIndex + 1) % this.listData.length
            const offsetHeightTotal = listDom.offsetHeight // 重新获取所有条目的总高度，以保证数据的即使有效性；
            const offsetHeight =
              listDom.children[this.currentItemIndex].offsetHeight // 重新获取单个条目的高度，以保证数据的即使有效性；
    ...
    hasNoRestMode(indi) {
      ...
      const boxHeight = listBoxDom.offsetHeight // list容器的高度；
      const offsetHeightTotal = listDom.offsetHeight // 获取所有条目的总高度；
      const offsetHeight = listDom.children[0].offsetHeight // 获取第一个条目的高度；
      if (offsetHeightTotal >= boxHeight) {
        // 所有条目的总高度 大于或等于 list容器的高度，才轮播；
        this.listCopyExistFlag = true
        if (indi === 'start') {
          ...
          this.loopTimer = setInterval(() => {
            ...
            const timeGap = endTime - this.startTime
...
.vue-list-marquee-list {
  ...
  > div {
    ...
    > div:nth-child(2n) {
      transform: translateZ(0);
      /*background: rgba(17, 72, 78, 0.348);*/
    }
  }
}
</style>
